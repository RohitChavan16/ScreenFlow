  ...
  try {
    ...
    return (
        ...
        <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-lg z-50 flex flex-col md:flex-row items-center max-md:justify-center gap-8 min-md:px-8 py-3 max-md:h-screen min-md:rounded-full backdrop-blur bg-black/70 md:bg-white/10 md:border border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen ? 'max-md:w-full' : 'max-md:w-0'}`}>
          ...
          <Link onClick = {() => {scrollTo(0, 0); setIsOpen(false)} } to='/' className="hover:[color:#fe5454]" >Home</Link>
          <Link onClick = {() => {scrollTo(0, 0); setIsOpen(false)}} to='/movies' className="hover:[color:#fe5454]" >Movies</Link>
          <Link onClick = {() => {scrollTo(0, 0); setIsOpen(false)}} to='/' className="hover:[color:#fe5454]" >Theaters</Link>
          <Link onClick = {() => {scrollTo(0, 0); setIsOpen(false)}} to='/my-bookings' className="hover:[color:#fe5454]" >My-Booking</Link>
          { favoriteMovies.length > 0 && <Link onClick = {() => {scrollTo(0, 0); setIsOpen(false)}} to='/favorite' className="hover:[color:#fe5454]" >Favorite</Link>}
        </div>
        <div className='flex items-center gap-8'>
         <SearchIcon className='max-md:hidden w-6 h-6 cursor-pointer'/>
     { userData ? <div className="w-8 h-8 flex justify-center items-center rounded-full bg-red-700 text-white relative group">
